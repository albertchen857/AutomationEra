...
import com.automationera.emi.category.TutorialRecipeCategory;
import com.automationera.emi.display.MachineEmiRecipe;
import com.automationera.emi.display.TradeEmiRecipe;
import com.automationera.emi.display.TutorialEmiRecipe;
...
public class emiPlugin implements EmiPlugin {
    ...
    public static final EmiRecipeCategory TUTORIAL_CATEGORY = new TutorialRecipeCategory();
    ...
    public void register(EmiRegistry registry) {
        ...
        registry.addCategory(TUTORIAL_CATEGORY);
        ...
        for (Map.Entry<String, List<Item>> entry : tut.entrySet()) {
            String key = entry.getKey();
            List<Item> output = entry.getValue();
            if (!output.isEmpty()) {
                registry.addRecipe(new TutorialEmiRecipe(new MachineRecipe(output.get(0), output.get(0)), key, output));
            }
        }
    }
}
